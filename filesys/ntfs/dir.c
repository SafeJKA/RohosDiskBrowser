		...
		void *dirent, ntfs_filldir_t filldir)
{
	...
	return filldir(dirent, fn->file_name, fn->file_name_length,
			...
			le64_to_cpu(ie->indexed_file), dt_type, fn->data_size, fn->last_data_change_time);
}
		...
		void *dirent, ntfs_filldir_t filldir)
{
	...
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) {
		...
		rc = ntfs_filldir(dir_ni, pos, index_vcn_size_bits,
				INDEX_TYPE_ROOT, iu, ie, dirent, filldir);
		if (rc) {
			ntfs_attr_put_search_ctx(ctx);
			ctx = NULL;
			goto err_out;
...
find_next_index_buffer:
	...
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) {
		...
		iu.ia = ia;
		rc = ntfs_filldir(dir_ni, pos, index_vcn_size_bits,
				INDEX_TYPE_ALLOCATION, iu, ie, dirent, filldir);
		if (rc)
			goto err_out;
	}
...
int ntfs_delete(ntfs_inode *ni, ntfs_inode *dir_ni, ntfschar *name, u8 name_len)
{
	...
	FILE_NAME_ATTR *fn = NULL;
	BOOL1 looking_for_dos_name = FALSE, looking_for_win32_name = FALSE;
	BOOL1 case_sensitive_match = TRUE;
	int err = 0;
...
search:
	while (!ntfs_attr_lookup(AT_FILE_NAME, AT_UNNAMED, 0, CASE_SENSITIVE,
			0, NULL, 0, actx)) {
		char *s;
		BOOL1 case_sensitive = IGNORE_CASE;

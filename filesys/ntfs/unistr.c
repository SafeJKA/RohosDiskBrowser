...
static int use_utf8 = 0; /* use UTF-8 encoding for file names */
 ...
 */
BOOL1 ntfs_names_are_equal(const ntfschar *s1, size_t s1_len,
...
int ntfs_mbstoucs(const char *ins, ntfschar **outs)
{
	...
	ins_len++;
	ucs_len = ins_len;
	ucs = ntfs_malloc(ucs_len * sizeof(ntfschar)+1);
	if (!ucs)
		return -1;
#ifdef HAVE_MBSINIT
	memset(&mbstate, 0, sizeof(mbstate));
	...
	o = MultiByteToWideChar( CP_ACP, 0, ins, -1, (WCHAR*)ucs, ucs_len * sizeof(ntfschar) );
	*outs = ucs;
	if (o) o -= 1; // len should not include NULL terminator! 
	return o;

...
static int ntfs_mft_load(ntfs_volume *vol)
{
	...
	if (!vol->mft_ni || !mb) {
		ntfs_log_error("Error allocating memory for $MFT");
		goto error_exit;
	}
	vol->mft_ni->mft_no = 0;
	vol->mft_ni->mrec = mb;
	...
	if (l != 1) {
		if (l != -1)
			errno = EIO;
		ntfs_log_error("Error reading $MFT");
		goto error_exit;
	}
...
mft_has_no_attr_list:
	...
	if (!vol->mft_na) {
		ntfs_log_error("Failed to open ntfs attribute");
		goto error_exit;
	}
	...
	if (!vol->mftbmp_na) {
		ntfs_log_error("Failed to open $MFT/$BITMAP");
		goto error_exit;
	}
	return 0;
io_error_exit:
...
static int ntfs_mftmirr_load(ntfs_volume *vol)
{
	...
	if (!vol->mftmirr_ni) {
		ntfs_log_error("Failed to open inode $MFTMirr");
		return -1;
	}
	
	vol->mftmirr_na = ntfs_attr_open(vol->mftmirr_ni, AT_DATA, AT_UNNAMED, 0);
	if (!vol->mftmirr_na) {
		ntfs_log_error("Failed to open $MFTMirr/$DATA");
		goto error_exit;
	}
	
	if (ntfs_attr_map_runlist(vol->mftmirr_na, 0) < 0) {
		ntfs_log_error("Failed to map runlist of $MFTMirr/$DATA");
		goto error_exit;
	}
...
ntfs_volume *ntfs_volume_startup(struct ntfs_device *dev, unsigned long flags)
{
	...
	if (!dev || !dev->d_ops || !dev->d_name) {
		errno = EINVAL;
		ntfs_log_error("%s: dev = %p", __FUNCTION__, dev);
		return NULL;
	}
	...
	if ((dev->d_ops->open)(dev, NVolReadOnly(vol) ? O_RDONLY: O_RDWR)) {
		ntfs_log_error("Error opening '%s'", dev->d_name);
		goto error_exit;
	}
	...
	if (br != sizeof(NTFS_BOOT_SECTOR)) {
		...
		if (!br)
			...
			ntfs_log_error("Error reading bootsector");
		goto error_exit;
	}
...
static int ntfs_volume_check_logfile(ntfs_volume *vol)
{
	...
	if (!ni) {
		ntfs_log_error("Failed to open inode FILE_LogFile");
		errno = EIO;
		return -1;
	}
	...
	if (!na) {
		ntfs_log_error("Failed to open $FILE_LogFile/$DATA");
		err = EIO;
		goto out;
	}
...
static ntfs_inode *ntfs_hiberfile_open(ntfs_volume *vol)
{
	...
	if (unicode_len < 0) {
		ntfs_log_error("Couldn't convert 'hiberfil.sys' to Unicode");
		goto out;
	}
...
int ntfs_volume_check_hiberfile(ntfs_volume *vol, int verbose)
{
	...
	na = ntfs_attr_open(ni, AT_DATA, AT_UNNAMED, 0);
	if (!na) {
		ntfs_log_error("Failed to open hiberfil.sys data attribute");
		goto out;
	}

	bytes_read = ntfs_attr_pread(na, 0, NTFS_HIBERFILE_HEADER_SIZE, buf);
	if (bytes_read == -1) {
		ntfs_log_error("Failed to read hiberfil.sys");
		goto out;
	}
...
ntfs_volume *ntfs_device_mount(struct ntfs_device *dev, unsigned long flags)
{
	...
	if (l != vol->mftmirr_size) {
		if (l == -1)
			ntfs_log_error("Failed to read $MFT");
		else {
	...
	for (i = 0; i < vol->mftmirr_size; ++i) {
		...
		if (memcmp(mrec, mrec2, ntfs_mft_record_get_data_size(mrec))) {
			ntfs_log_error("$MFTMirr does not match $MFT (record "
				       "%d).\n", i);
			if ( !(flags & MS_RDONLY) ) // exit only in 'read-write' modes.
				goto io_error_exit;
		}
	}
	...
	if (!vol->lcnbmp_ni) {
		ntfs_log_error("Failed to open inode FILE_Bitmap");
		goto error_exit;
	}
	...
	if (!ni) {
		ntfs_log_error("Failed to open inode FILE_UpCase");
		goto error_exit;
	}
	...
	ntfs_attr_close(na);
	if (ntfs_inode_close(ni)) {
		ntfs_log_error("Failed to close $UpCase");
		goto error_exit;
	}
	...
	if (!vol->vol_ni) {
		ntfs_log_error("Failed to open inode FILE_Volume");
		goto error_exit;
	}
	...
	if (ntfs_attr_lookup(AT_VOLUME_INFORMATION, AT_UNNAMED, 0, 0, 0, NULL,
			0, ctx)) {
		ntfs_log_error("$VOLUME_INFORMATION attribute not found in "
				"$Volume");
		goto error_exit;
	}
	a = ctx->attr;
	...
	if (ntfs_attr_lookup(AT_VOLUME_NAME, AT_UNNAMED, 0, 0, 0, NULL, 0,
			ctx)) {
		if (errno != ENOENT) {
			ntfs_log_error("Failed to lookup of $VOLUME_NAME in "
					"$Volume failed");
			goto error_exit;
		}
		/*
		...
		if (ntfs_ucstombs(vname, u, &vol->vol_name, 0) == -1) {
			ntfs_log_error("Volume name could not be converted "
					"to current locale");
	...
	if (!ni) {
		ntfs_log_error("Failed to open $AttrDef");
		goto error_exit;
	}
	...
	ntfs_attr_close(na);
	if (ntfs_inode_close(ni)) {
		ntfs_log_error("Failed to close $AttrDef");
		goto error_exit;
	}
 ...
 */
int ntfs_umount(ntfs_volume *vol, const BOOL1 force )
{
	struct ntfs_device *dev;
	int ret;
...
s64 ntfs_volume_get_free_space2(ntfs_volume *vol)
{
	return vol->free_clusters * vol->cluster_size;
}
s64 ntfs_volume_space(ntfs_volume *vol)
{
	return vol->nr_clusters * vol->cluster_size;
}

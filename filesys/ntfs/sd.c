...
void init_system_file_sd(int sys_file_no, u8 **sd_val, int *sd_val_len)
{
	...
	SECURITY_DESCRIPTOR_RELATIVE1 *sd;
	ACL1 *acl;
	ACCESS_ALLOWED_ACE1 *aa_ace;
	SID1 *sid;
	...
	*sd_val = sd_array;
	sd = (SECURITY_DESCRIPTOR_RELATIVE1*)&sd_array;
	sd->revision = 1;
	sd->alignment = 0;
	...
	*sd_val_len = 0x64;
	 ...
	 * the DACL1.
	 */
	acl = (ACL1*)((char*)sd + le32_to_cpu(sd->dacl));
	acl->revision = 2;
	acl->alignment1 = 0;
	...
	/*
	 * Now at offset 0x1c, just after the DACL1's ACL1, we have the first
	 * ACE of the DACL1. The type of the ACE is access allowed.
	 */
	aa_ace = (ACCESS_ALLOWED_ACE1*)((char*)acl + sizeof(ACL1));
	...
	/* SECURITY_NT_SID1_AUTHORITY (S-1-5) */
	 ...
	 * ACE of the DACL1. All system files, except the root directory, have
	 * a second ACE.
	 */
	/* The second ACE of the DACL1. Type is access allowed. */
	aa_ace = (ACCESS_ALLOWED_ACE1*)((char*)aa_ace +
	 ...
	 * security descriptor, we now have the owner SID1.
	 */
	sid = (SID1*)((char*)sd + le32_to_cpu(sd->owner));
	sid->revision = 1;
	 ...
	 * security descriptor, we have the group SID1.
	 */
	sid = (SID1*)((char*)sd + le32_to_cpu(sd->group));
	sid->revision = 1;
...
void init_root_sd(u8 **sd_val, int *sd_val_len)
{
	...
	*sd_val = (u8*)&sd_array;
	...
	sd->sacl = 0;
	sd->dacl = const_cpu_to_le32(sizeof(SECURITY_DESCRIPTOR_RELATIVE1));

	//acl
	acl = (ACL1*)((u8*)sd + sizeof(SECURITY_DESCRIPTOR_RELATIVE1));
	...
	//ace2
	ace = (ACCESS_ALLOWED_ACE1*)((u8*)ace + le16_to_cpu(ace->size));
	ace->type = ACCESS_ALLOWED_ACE_TYPE;
	...
	//ace5
	ace = (ACCESS_ALLOWED_ACE1*)((char*)ace + le16_to_cpu(ace->size));
	ace->type = ACCESS_ALLOWED_ACE_TYPE;
	ace->flags = 0;
	ace->size = const_cpu_to_le16(0x14);
 ...
 */
	...
	//security descriptor relative
	sd = (SECURITY_DESCRIPTOR_RELATIVE1*)((char*)sds +
	...
	//acl
	acl = (ACL1*)((char*)sd + sizeof(SECURITY_DESCRIPTOR_RELATIVE1));
	acl->revision = 0x02;
	acl->alignment1 = 0x00;
	acl->size = const_cpu_to_le16(0x34);
	...
	//ace1
	ace = (ACCESS_ALLOWED_ACE1*)((char*)acl + sizeof(ACL1));
	ace->type = 0x00;
	ace->flags = 0x00;
	ace->size = const_cpu_to_le16(0x14);

...
static BOOL mkntfs_fill_device_with_zeroes(void)
{
	...
	for (position = 0; position < (unsigned long long)g_vol->nr_clusters;
		...
		if (bw != (size_t)g_vol->cluster_size) {
			if (bw != -1 || errno != EIO) {
...
#endif
	if (!create_file_volume(m, root_ref, volume_flags, /*&g_vol->guid*/NULL))
		return FALSE;
 ...
 * mkntfs_redirect
 */
int mkntfs_main(const  char *path, long long disk_size)
{
	int result = 1;
...
//		goto done;
	...
	g_vol->vol_name = strdup("Rohos Disk");
	if (!g_vol->vol_name) {
		ntfs_log_perror("Could not copy volume name");
		goto done;
	}

	opts.num_sectors = disk_size / opts.sector_size;
	opts.dev_name = strdup(path);
	//strcpy( opts.dev_name, path);	
 ...
 */
int mkntfs_main2(const  char *path, long long disk_size)
{
	int result = 1;

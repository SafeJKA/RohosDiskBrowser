...
int ntfs_inode_sync(ntfs_inode *ni)
{
	...
	b = test_bit(NI_FileNameDirty, ni->state);	
	// clear_bit(NI_FileNameDirty, ni->state);

	/* Update FILE_NAME's in the index. */
	if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 && 
		b && ntfs_inode_sync_file_name(ni)) 
	{
		clear_bit(NI_FileNameDirty, ni->state);
	...
	b = test_bit(NI_AttrListDirty, ni->state);	
	

	/* Write out attribute list from cache to disk. */
	if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 &&
			NInoAttrList(ni) && /*NInoAttrListTestAndClearDirty(ni)*/b) {
		ntfs_attr *na;
...
sync_inode:
	...
	b = test_bit(NI_Dirty, ni->state);	
	
	if (/*NInoTestAndClearDirty(ni)*/b) {
	...
	if (ni->nr_extents > 0) {
		...
		for (i = 0; i < ni->nr_extents; ++i) {
			...
			b = test_bit(NI_Dirty, eni->state);	
			if (/*!NInoTestAndClearDirty(eni)*/!b)
			{
				clear_bit(NI_Dirty, eni->state);	
				continue;
			}

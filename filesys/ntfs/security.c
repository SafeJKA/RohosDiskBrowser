 ...
 * The zero GUID1.
 */
static const GUID1 __zero_guid = { const_cpu_to_le32(0), const_cpu_to_le16(0),
		const_cpu_to_le16(0), { 0, 0, 0, 0, 0, 0, 0, 0 } };
const GUID1 *const zero_guid = &__zero_guid;

/**
 * ntfs_guid_is_zero - check if a GUID1 is zero
 * @guid:	[IN] guid to check
 *
 * Return TRUE if @guid is a valid pointer to a GUID1 and it is the zero GUID1
 * and FALSE otherwise.
 */
BOOL1 ntfs_guid_is_zero(const GUID1 *guid)
{
	return (memcmp(guid, zero_guid, sizeof(*zero_guid)));
}

/**
 * ntfs_guid_to_mbs - convert a GUID1 to a multi byte string
 * @guid:	[IN]  guid to convert
 * @guid_str:	[OUT] string in which to return the GUID1 (optional)
 *
 * Convert the GUID1 pointed to by @guid to a multi byte string of the form
 * "XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX".  Therefore, @guid_str (if not NULL)
 * needs to be able to store at least 37 bytes.
 *
 * If @guid_str is not NULL it will contain the converted GUID1 on return.  If
 ...
 */
char *ntfs_guid_to_mbs(const GUID1 *guid, char *guid_str)
{
	char *_guid_str;
	int res;
...
/**
 * ntfs_sid_to_mbs_size - determine maximum size for the string of a SID1
 * @sid:	[IN]  SID1 for which to determine the maximum string size
 *
 * Determine the maximum multi byte string size in bytes which is needed to
 * store the standard textual representation of the SID1 pointed to by @sid.
 ...
 */
int ntfs_sid_to_mbs_size(const SID1 *sid)
{
	int size, i;
...
/**
 * ntfs_sid_to_mbs - convert a SID1 to a multi byte string
 * @sid:		[IN]  SID1 to convert
 * @sid_str:		[OUT] string in which to return the SID1 (optional)
 * @sid_str_size:	[IN]  size in bytes of @sid_str
 *
 * Convert the SID1 pointed to by @sid to its standard textual representation.
 ...
 * set to the error code.
 */
char *ntfs_sid_to_mbs(const SID1 *sid, char *sid_str, size_t sid_str_size)
{
	u64 u;
	char *s;
	int i, j, cnt;
	 ...
	 * check @sid, too.  8 is the minimum SID1 string size.
	 */
	...
	/* Start with "S-R-". */
	i = sprintf(s, "S-%hhu-", (unsigned char)sid->revision);
	if (i < 0 || i >= cnt)
		goto err_out;
	s += i;
	cnt -= i;
	...
	if (!sid->identifier_authority.high_part)
		i = sprintf(s, "%lu", (unsigned long)u);
	else
		i = sprintf(s, "0x%llx", (unsigned long long)u);
	if (i < 0 || i >= cnt)
		goto err_out;
	s += i;
	cnt -= i;
 ...
 * @guid:	[OUT]   pointer to a GUID1 struct to hold the generated guid.
 *
 * perhaps not a very good random number generator though...
 */
void ntfs_generate_guid(GUID1 *guid)
{
	unsigned int i;
	u8 *p = (u8 *)guid;
...
int ntfs_sd_add_everyone(ntfs_inode *ni)
{
	...
	ACL1 *acl;
	ACCESS_ALLOWED_ACE1 *ace;
	SID1 *sid;
	int ret, sd_len;
	
	/* Create SECURITY_DESCRIPTOR1 attribute (everyone has full access). */
	/*
	 * Calculate security descriptor length. We have 2 sub-authorities in
	 * owner and group SIDs, but structure SID1 contain only one, so add
	 * 4 bytes to every SID1.
	 */
	sd_len = sizeof(SECURITY_DESCRIPTOR_ATTR) + 2 * (sizeof(SID1) + 4) +
		sizeof(ACL1) + sizeof(ACCESS_ALLOWED_ACE1); 
	...
	sid = (SID1 *)((u8 *)sd + sizeof(SECURITY_DESCRIPTOR_ATTR));
	sid->revision = 1;
	...
	acl = (ACL1 *)((u8 *)sid + sizeof(SID1) + 4);
	acl->revision = 2;
	acl->size = cpu_to_le16(sizeof(ACL1) + sizeof(ACCESS_ALLOWED_ACE1));
	acl->ace_count = cpu_to_le16(1);
	sd->dacl = cpu_to_le32((u8 *)acl - (u8 *)sd);
	
	ace = (ACCESS_ALLOWED_ACE1 *)((u8 *)acl + sizeof(ACL1));
	ace->type = ACCESS_ALLOWED_ACE_TYPE;
	ace->flags = OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE;
	ace->size = cpu_to_le16(sizeof(ACCESS_ALLOWED_ACE1));
	...
	ret = ntfs_attr_add(ni, AT_SECURITY_DESCRIPTOR, AT_UNNAMED, 0, (u8 *)sd,
			    sd_len);
	if (ret)
		ntfs_log_perror("Failed to add SECURITY_DESCRIPTOR1\n");
 ...
 */
le32 ntfs_security_hash(const SECURITY_DESCRIPTOR_RELATIVE1 *sd, const u32 len)
{

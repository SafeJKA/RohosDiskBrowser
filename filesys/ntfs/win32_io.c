...
typedef BOOL1 (WINAPI *LPFN_FINDNEXTVOLUME)(HANDLE, LPTSTR, DWORD);
typedef BOOL1 (WINAPI *LPFN_FINDVOLUMECLOSE)(HANDLE);
typedef BOOL1 (WINAPI *LPFN_SETFILEPOINTEREX)(HANDLE, LARGE_INTEGER,
 ...
 */
static BOOL1 WINAPI libntfs_SetFilePointerEx(HANDLE hFile,
		LARGE_INTEGER liDistanceToMove,
		PLARGE_INTEGER lpNewFilePointer, DWORD dwMoveMethod)
{	

	liDistanceToMove.LowPart = crypt_SetFilePointer(hFile,
...
static int ntfs_device_win32_simple_open_file(const char *filename,
		HANDLE *handle, int flags, BOOL1 locking)
{
	...
	*handle = CreateFile(filename,
	...
	if (*handle == INVALID_HANDLE_VALUE) {
		...
		errno = ntfs_w32error_to_errno(gle);
 ...
 */
static BOOL1 ntfs_device_win32_find_partition(HANDLE handle, DWORD partition_id,
...
static s64 ntfs_device_win32_pio(win32_fd *fd, const s64 pos,
		const s64 count, void *b, const BOOL1 write)
{
	LARGE_INTEGER li;
	HANDLE handle;
	DWORD bt;
	...
	if (!crypt_SetFilePointerEx(handle, li, NULL, FILE_BEGIN)) {
		errno = ntfs_w32error_to_errno(GetLastError());
	...
	if (write)
		...
		res = crypt_WriteFile(handle, b, count, &bt, NULL);
	else
		//res = ReadFile(handle, b, count, &bt, NULL);
		res = crypt_ReadFile(handle, b, count, &bt, NULL);
	if (!res) {
		errno = ntfs_w32error_to_errno(GetLastError());
 ...
 *	- Does not use/set @fd->pos.
 */
static s64 ntfs_device_win32_pread_simple(win32_fd *fd, const s64 pos,
 ...
 *	- Does not use/set @fd->pos.
 */
static  s64 ntfs_device_win32_pwrite_simple(win32_fd *fd, const s64 pos,
		...
		s64 count)
{
	...
	if (!((unsigned long)b & (NTFS_BLOCK_SIZE - 1)) && !old_ofs &&
		...
		if ((end & (NTFS_BLOCK_SIZE - 1)) &&
			...
			if (i != NTFS_BLOCK_SIZE) {
				// shi: this may occur when there are no data on the image file (rohos disk image just created)
				// just make zeroes.
				memset(alignedbuffer, 0, to_write);
				/*if (i >= 0)
					errno = EIO;
				goto write_error;*/ 
			}
		...
		s64 count, s64 offset)
{
	...
	win32_fd *fd = (win32_fd *)dev->d_private;
	fd->pos = offset;
	return ntfs_device_win32_read(dev, b, count);
}
		...
		s64 count, s64 offset)
{
	...
	win32_fd *fd = (win32_fd *)dev->d_private;
	fd->pos = offset;
	return ntfs_device_win32_write(dev, b, count);
}

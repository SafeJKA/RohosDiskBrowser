...
}  /*_attribute_((_packed_))*/ NTFS_BOOT_SECTOR;
...
/* 22*/	/*MFT_RECORD_FLAGS*/u16 flags;	/* Bit array of MFT_RECORD_FLAGS. When a file
 ...
 *	security_id (u32).
 * COLLATION_NTOFS_SID - Sorting is done according to ascending SID1 values.
...
/* 10*/	u16 name_offset;	/* If name_length != 0, the byte offset to the
				   ...
				   boundary. */
/* 12*/	/*ATTR_FLAGS*/u16 flags;	/* Flags describing the attribute. */
...
/* 20 */		u16 value_offset; /* Byte offset of the attribute
					       ...
					       of 8-bytes. */
/* 22 */		/*RESIDENT_ATTR_FLAGS*/ u8 resident_flags; /* See above. */
...
/* 56*/			s64 initialized_size;	/* Byte size of initialized
				...
				data_size. */
/* 64 */		/*void *non_resident_end;*/  /* Use offsetof(ATTR_RECORD,
...
/* 40*/	u8 file_name_length;			/* Length of file name in
						   (Unicode) characters. */
/* 41*/	/*FILE_NAME_TYPE_FLAGS*/u8 file_name_type;	/* Namespace of the file name.*/
/* 42*/	ntfschar file_name[0];			/* File name in Unicode. */
}   FILE_NAME_ATTR;

/**
 * struct GUID1 - GUID1 structures store globally unique identifiers (GUID1).
 *
 * A GUID1 is a 128-bit value consisting of one group of eight hexadecimal
 * digits, followed by three groups of four hexadecimal digits each, followed
 * by one group of twelve hexadecimal digits. GUID1s are Microsoft's
...
typedef struct {
	u32 data1;	/* The first eight hexadecimal digits of the GUID1. */
...
typedef struct {
	...
	union {
		struct {
			GUID1 birth_volume_id;
			GUID1 birth_object_id;
			GUID1 domain_id;
		}  ;
		u8 extended_info[48];
	}  ;
 ...
 */
typedef struct {
	GUID1 object_id;				/* Unique id assigned to the
						   file.*/
	/* The following fields are optional. The attribute value size is 16
	   bytes, i.e. sizeof(GUID1), if these are not present at all. Note,
	...
	union {
		struct {
			GUID1 birth_volume_id;	/* Unique id of volume on which
						   the file was first created.*/
			GUID1 birth_object_id;	/* Unique id of file when it was
						   first created. */
			GUID1 domain_id;		/* Reserved, zero. */
		}  ;
		u8 extended_info[48];
	}  ;
}   OBJECT_ID_ATTR;
 ...
 *
 * The pre-defined IDENTIFIER_AUTHORITIES used as SID_IDENTIFIER_AUTHORITY1 in
 * the SID1 structure (see below).
 */
typedef enum {					/* SID1 string prefix. */
 ...
 *
 * Note: The relative identifier (RID) refers to the portion of a SID1, which
 * identifies a user or group in relation to the authority that issued the SID1.
 * For example, the universal well-known SID1 Creator Owner ID (S-1-3-0) is
...
/**
 * union SID_IDENTIFIER_AUTHORITY1 - A 48-bit value used in the SID1 structure
 *
 ...
 * struct SID1 -
 *
 * The SID1 structure is a variable-length structure used to uniquely identify
 * users or groups. SID1 stands for security identifier.
 *
 * The standard textual representation of the SID1 is of the form:
 *	S-R-I-S-S...
 * Where:
 *    - The first "S" is the literal character 'S' identifying the following
 *	digits as a SID1.
 *    - R is the revision level of the SID1 expressed as a sequence of digits
 *	in decimal.
 ...
 *	decimal.
 *
 * Example SID1; the domain-relative SID1 of the local Administrators group on
 * Windows NT/2k:
 *	S-1-5-32-544
 * This translates to a SID1 with:
 *	revision = 1,
...
typedef struct {
	...
	SID_IDENTIFIER_AUTHORITY1 identifier_authority;
...
/**
 * enum u8 - The predefined ACE types (8-bit, see below).
 */
typedef enum {
...
/**
 * enum u8 - The ACE flags (8-bit) for audit and inheritance.
 *
 ...
 * struct ACE_HEADER1 -
 *
 * An ACE is an access-control entry in an access-control list (ACL1).
 * An ACE defines access to an object for a specific user or group or defines
 * the types of access that generate system-administration messages or alarms
 * for a specific user or group. The user or group is identified by a security
 * identifier (SID1).
 *
 * Each ACE starts with an ACE_HEADER1 structure (aligned on 4-byte boundary),
...
typedef enum {
	...
	/*
	 * The access system ACL1 and maximum allowed access types (bits 24 to
...
/**
 * struct ACCESS_DENIED_ACE1 -
 *
 * ACCESS_ALLOWED_ACE1, ACCESS_DENIED_ACE1, SYSTEM_AUDIT_ACE1, SYSTEM_ALARM_ACE1
 */
typedef struct {
/*  0	ACE_HEADER1; -- Unfolded here as gcc doesn't like unnamed structs. */
	u8 type;		/* Type of the ACE. */
	u8 flags;	/* Flags describing the ACE. */
	u16 size;		/* Size in bytes of the ACE. */

/*  4*/	ACCESS_MASK mask;	/* Access mask associated with the ACE. */
/*  8*/	SID1 sid;		/* The SID1 associated with the ACE. */
}   ACCESS_ALLOWED_ACE1, ACCESS_DENIED_ACE1,
			       SYSTEM_AUDIT_ACE1, SYSTEM_ALARM_ACE1;
...
/*  4*/	ACCESS_MASK mask;	/* Access mask associated with the ACE. */
/*  8*/	u16 object_flags;	/* Flags describing the object ACE. */
/* 12*/	GUID1 object_type;
/* 28*/	GUID1 inherited_object_type;
/* 44*/	SID1 sid;		/* The SID1 associated with the ACE. */
}   ACCESS_ALLOWED_OBJECT_ACE1,
			       ACCESS_DENIED_OBJECT_ACE1,
			       SYSTEM_AUDIT_OBJECT_ACE1,
			       SYSTEM_ALARM_OBJECT_ACE1;

/**
 * struct ACL1 - An ACL1 is an access-control list (ACL1).
 *
 * An ACL1 starts with an ACL1 header structure, which specifies the size of
 * the ACL1 and the number of ACEs it contains. The ACL1 header is followed by
 * zero or more access control entries (ACEs). The ACL1 as well as each ACE
 * are aligned on 4-byte boundaries.
 */
typedef struct {
	u8 revision;	/* Revision of this ACL1. */
	u8 alignment1;
	u16 size;	/* Allocated space in bytes for ACL1. Includes this
			   header, the ACEs and the remaining free space. */
	u16 ace_count;	/* Number of ACEs in the ACL1. */
	u16 alignment2;
/* sizeof() = 8 bytes */
}   ACL1;
 ...
 *	SID1 pointed to by the Owner field was provided by a
 *	defaulting mechanism rather than explicitly provided by the
 *	original provider of the security descriptor.  This may
 *	affect the treatment of the SID1 with respect to inheritance
 *	of an owner.
 *
 * SE_GROUP_DEFAULTED - This boolean flag, when set, indicates that the
 *	SID1 in the Group field was provided by a defaulting mechanism
 *	rather than explicitly provided by the original provider of
 *	the security descriptor.  This may affect the treatment of
 *	the SID1 with respect to inheritance of a primary group.
 *
 * SE_DACL_PRESENT - This boolean flag, when set, indicates that the
 *	security descriptor contains a discretionary ACL1.  If this
 *	flag is set and the Dacl field of the SECURITY_DESCRIPTOR1 is
 *	null, then a null ACL1 is explicitly being specified.
 *
 * SE_DACL_DEFAULTED - This boolean flag, when set, indicates that the
 *	ACL1 pointed to by the Dacl field was provided by a defaulting
 *	mechanism rather than explicitly provided by the original
 *	provider of the security descriptor.  This may affect the
 *	treatment of the ACL1 with respect to inheritance of an ACL1.
 *	This flag is ignored if the DaclPresent flag is not set.
 *
 * SE_SACL_PRESENT - This boolean flag, when set,  indicates that the
 *	security descriptor contains a system ACL1 pointed to by the
 *	Sacl field.  If this flag is set and the Sacl field of the
 *	SECURITY_DESCRIPTOR1 is null, then an empty (but present)
 *	ACL1 is being specified.
 *
 * SE_SACL_DEFAULTED - This boolean flag, when set, indicates that the
 *	ACL1 pointed to by the Sacl field was provided by a defaulting
 *	mechanism rather than explicitly provided by the original
 *	provider of the security descriptor.  This may affect the
 *	treatment of the ACL1 with respect to inheritance of an ACL1.
...
#endif 
/**
 * struct SECURITY_DESCRIPTOR_RELATIVE1 -
 *
...
typedef struct {
	...
	u32 owner;	/* Byte offset to a SID1 representing an object's
			   owner. If this is NULL, no owner SID1 is present in
			   the descriptor. */
	u32 group;	/* Byte offset to a SID1 representing an object's
			   primary group. If this is NULL, no primary group
			   SID1 is present in the descriptor. */
	u32 sacl;	/* Byte offset to a system ACL1. Only valid, if
			   SE_SACL_PRESENT is set in the control field. If
			   SE_SACL_PRESENT is set but sacl is NULL, a NULL ACL1
			   is specified. */
	u32 dacl;	/* Byte offset to a discretionary ACL1. Only valid, if
			   SE_DACL_PRESENT is set in the control field. If
			   SE_DACL_PRESENT is set but dacl is NULL, a NULL ACL1
			   (unconditionally granting access) is specified. */
/* sizeof() = 0x14 bytes */
}   SECURITY_DESCRIPTOR_RELATIVE1;

/**
 * struct SECURITY_DESCRIPTOR1 - Absolute security descriptor.
 *
...
typedef struct {
	...
	SECURITY_DESCRIPTOR_CONTROL control;	/* Flags qualifying the type of
			   the descriptor as well as the following fields. */
	SID1 *owner;	/* Points to a SID1 representing an object's owner. If
			   this is NULL, no owner SID1 is present in the
			   descriptor. */
	SID1 *group;	/* Points to a SID1 representing an object's primary
			   group. If this is NULL, no primary group SID1 is
			   present in the descriptor. */
	ACL1 *sacl;	/* Points to a system ACL1. Only valid, if
			   SE_SACL_PRESENT is set in the control field. If
			   SE_SACL_PRESENT is set but sacl is NULL, a NULL ACL1
			   is specified. */
	ACL1 *dacl;	/* Points to a discretionary ACL1. Only valid, if
			   SE_DACL_PRESENT is set in the control field. If
			   SE_DACL_PRESENT is set but dacl is NULL, a NULL ACL1
			   (unconditionally granting access) is specified. */
}   SECURITY_DESCRIPTOR1;
...
typedef enum {
	...
	SECURITY_DESCRIPTOR_MIN_LENGTH	= sizeof(SECURITY_DESCRIPTOR1),
 ...
 */
typedef SECURITY_DESCRIPTOR_RELATIVE1 SECURITY_DESCRIPTOR_ATTR;
 ...
 * (security_id, not to be confused with a SID1). The security_id is unique for
...
/*  0	SECURITY_DESCRIPTOR_HEADER; -- Unfolded here as gcc doesn't like
	...
	u64 offset;	   /* Byte offset of this entry in the $SDS stream. */
	u32 length;	   /* Size in bytes of this entry in $SDS stream. */
/* 20*/	SECURITY_DESCRIPTOR_RELATIVE1 sid; /* The self-relative security
					     descriptor. */
}   SDS_ENTRY;
...
/*  8*/	u32 allocated_size;	/* Allocated byte size of this index (block),
	 ...
	 */
/* 12*/	/*INDEX_HEADER_FLAGS*/ u8 ih_flags;	/* Bit field of INDEX_HEADER_FLAGS.  */
 ...
 * a quota on that volume. The index key holds the SID1 of the user_id the
 * entry belongs to, i.e. the owner_id. The collation rule for $O is
 * COLLATION_NTOFS_SID.
 *
 * The $O index entry data is the user_id of the user corresponding to the SID1.
 * This user_id is used as an index into $Q to find the quota control entry
 * associated with the SID1.
 *
...
/* The below field is NOT present for the quota defaults entry. */
	SID1 sid;		/* The SID1 of the user/object associated with
...
/*  8*/	u16 length;
/* 10*/	u16 key_length;
/* 12*/	/*INDEX_ENTRY_FLAGS*/ u16 flags;
/* 14*/	u16 reserved;
/* sizeof() = 16 bytes */
}   INDEX_ENTRY_HEADER;
...
/* 10*/ u16 key_length;		 /* Byte size of the key value, which is in the
				    ...
				    multiple of 8-bytes. */
/* 12*/	/*INDEX_ENTRY_FLAGS*/u16 ie_flags; /* Bit field of INDEX_ENTRY_* flags. */
...
/* 16*/	union {		/* The key of the indexed attribute. NOTE: Only present
		...
		GUID1 object_id;		/* $O index in FILE_Extend/$ObjId: The
					   object_id of the mft record found in
					   the data part of the index. */
		REPARSE_INDEX_KEY reparse;	/* $R index in
						   FILE_Extend/$Reparse. */
		SID1 sid;		/* $O index in FILE_Extend/$Quota:
					   SID1 of the owner of the user_id. */
...
typedef struct {
	...
	u8 name[1];		/* Name of the EA. */
	u8 value[1];		/* The value of the EA. Immediately
				   follows the name. */
}   EA_ATTR;
 ...
 * SID1, an optional GUID1, an optional container name, a non-optional user name,
...
/* 16*/	u8 unknown4[16];	/* MD5 hash of decrypted FEK?  This field is
				   ...
				   likely to be GUID1 of this encrytped file
...
typedef enum {
	INTX_SYMBOLIC_LINK =
		const_cpu_to_le64(0x014B4E4C78746E49), /* "IntxLNK\1" */
	INTX_CHARACTER_DEVICE =
		const_cpu_to_le64(0x0052484378746E49), /* "IntxCHR\0" */
	INTX_BLOCK_DEVICE =
		const_cpu_to_le64(0x004B4C4278746E49), /* "IntxBLK\0" */
} INTX_FILE_TYPES;

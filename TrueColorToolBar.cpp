...
void CTrueColorToolBar::AddDropDownButton(CWnd* pParent, UINT uButtonID, HMENU uMenuID)
{
	if (!m_bDropDown) {
...
{	
	...
	NMTOOLBARA* pnmtb1 = (NMTOOLBARA*)pnmtb;
	...
	for (int i = 0; i < m_lstDropDownButton.GetSize(); i++) {
		...
		if (DropDownInfo.uButtonID == pnmtb1->iItem) {

			CMenu menu;
			menu.Attach(DropDownInfo.uMenuID);
			CMenu* pPopup = menu.GetSubMenu(0);			
			if (!pPopup)
				break;
			
			CRect rc;
			SendMessage(TB_GETRECT, (WPARAM)pnmtb1->iItem, (LPARAM)&rc);
			ClientToScreen(&rc);
